...
function memulai_pengecekan() {
    ...
    for user in "${data[@]}"; do
        ...
        local raw_limit_ip=$(cat /etc/limit/vmess/ip/"$user" 2>/dev/null || cat /etc/limit/vmess/ip/"$user_lower" 2>/dev/null || echo 0)
        local limit_ip=$(echo "$raw_limit_ip" | grep -o '[0-9]\+')
        [[ -z "$limit_ip" ]] && limit_ip=0
        
        # Membaca basis data kuota limit dan pemakaian
        local file_limit="/etc/vmess/$user"
        [[ ! -f "$file_limit" ]] && file_limit="/etc/vmess/$user_lower"
        
        local file_usage="/etc/limit/vmess/$user"
        [[ ! -f "$file_usage" ]] && file_usage="/etc/limit/vmess/$user_lower"
